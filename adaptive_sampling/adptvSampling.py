...
def writeOPF(l,T, myn_clusters, n_samples):
    import json
    OPF = []
    ...
    json.dump(OPF, open("lessPopClsInf.txt",'w'))
    return OPF
...
def CpptrajInGen():
    import json
    inf = json.load(open("lessPopClsInf.txt"))
    n_samples = len(inf)
    for i in range(n_samples):
            trj = inf[i]['traj']
            ...
            f.write('parmwrite out SampleR4_' + str(i) +'\n')
            f.write('run \n')
            f.write('quit')
            f.close()
